...
from util.common import open_folder  # You might not need this in API
...
def start_tryon(human_img_file, garm_img_file, garment_des, category, is_checked, is_checked_crop, denoise_steps, is_randomize_seed, seed, number_of_images):
    ...
    if is_checked:
        ...
        if not mask_file:
            return jsonify({"error": "Mask image is required when not using auto-masking"}), 400
        ...
        with torch.no_grad(), torch.cuda.amp.autocast(dtype=dtype), torch.inference_mode():
            ...
            for img in images:
                if is_checked_crop:
                    ...
                    buffered = BytesIO()
                    human_img_orig.save(buffered, format="PNG")
    ...
    mask_gray_base64 = base64.b64encode(buffered_mask.getvalue()).decode('utf-8')

    return jsonify({"generated_images": results, "masked_image": mask_gray_base64})
...
def tryon_endpoint():
    if request.method == 'POST':
        ...
        try:
            ...
            result = start_tryon(
                human_img_file, garm_img_file, garment_des, category, is_checked, is_checked_crop,
