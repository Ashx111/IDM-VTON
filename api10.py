...
def load_models():
    ...
    global pipe, unet, UNet_Encoder, parsing_model, openpose_model, tensor_transfrom # Added tensor_transform here
    
    # --- Configuration (Matching Gradio --load_mode 8bit) ---
    dtype = torch.float16         # Primary operating dtype for the pipeline and some components
    load_mode = '8bit'            # Explicitly match the Gradio flag
    fixed_vae = True              # Matches Gradio default (can be changed if needed)
    # --- End Configuration ---

    model_id = 'yisol/IDM-VTON'
    vae_model_id = 'madebyollin/sdxl-vae-fp16-fix'

    # Determine the dtype for quantized models based on load_mode
    if load_mode in ('4bit', '8bit'):
        print(f"Using quantization mode: {load_mode}")
        # NOTE: float8 support can be sensitive to environment (CUDA/PyTorch version)
        dtypeQuantize = torch.float8_e4m3fn
        if load_mode == '4bit':
             # Make sure quantize_4bit is imported if needed
             from util.pipeline import quantize_4bit
             print("Will use 4bit quantization where applicable.")
    else:
        print("No quantization specified, using float16 for all.")
        dtypeQuantize = dtype # Fallback to float16 if no load_mode specified

    print(f"Loading unet with dtype: {dtypeQuantize}")
    unet = UNet2DConditionModel.from_pretrained(
        model_id,
        subfolder="unet",
        torch_dtype=dtypeQuantize, # Use float8 for 8bit mode
    ).to(device)
    # Apply 4bit quantization if specified (AFTER loading with float8/float16)
    if load_mode == '4bit':
        print("Quantizing unet to 4bit")
        quantize_4bit(unet)
    unet.requires_grad_(False)

    # Image Encoder is usually kept at float16 even in mixed-precision setups
    print(f"Loading image_encoder with dtype: {torch.float16}")
    ...
    if load_mode == '4bit':
         print("Quantizing image_encoder to 4bit")
         quantize_4bit(image_encoder)
    image_encoder.requires_grad_(False)

    # VAE loading (usually float16 or float32)
    vae_load_dtype = dtype # Use primary float16 dtype
    ...
    # UNet_Encoder follows the same quantization as unet
    print(f"Loading UNet_Encoder with dtype: {dtypeQuantize}")
    ...
    if load_mode == '4bit':
        print("Quantizing UNet_Encoder to 4bit")
    ...
    pipe = TryonPipeline.from_pretrained(**pipe_param) # Components are already loaded and on device
    pipe.unet_encoder = UNet_Encoder # Assign the loaded encoder

    # Move the entire pipe object - this ensures text_encoders etc. are moved if they exist
    # and confirms device placement for already moved components.
    pipe.to(device)
    print("Pipeline components confirmed on device.")

    # Quantize text encoders AFTER moving pipe to device if using 4bit
    if load_mode == '4bit':
        print("Quantizing text encoders to 4bit (if they exist)")
        if hasattr(pipe, 'text_encoder') and pipe.text_encoder is not None:
            # Ensure text_encoder is on device before quantizing
            pipe.text_encoder.to(device)
            quantize_4bit(pipe.text_encoder)
        if hasattr(pipe, 'text_encoder_2') and pipe.text_encoder_2 is not None:
             # Ensure text_encoder_2 is on device before quantizing
